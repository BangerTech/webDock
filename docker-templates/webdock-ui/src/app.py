...
def setup_filestash(container_name, install_path, config_data=None):
    ...
    try:
        ...
        os.makedirs(data_dir, exist_ok=True, mode=0o755)
        os.makedirs(state_dir, exist_ok=True, mode=0o755)
        ...
        # Create temporary docker-compose file for initial setup
        before_compose_file = os.path.join(install_path, 'docker-compose-before.yml')
        with open(before_compose_file, 'w') as f:
            f.write(f"""version: '3'
services:
  app:
...
""")
        ...
        # Create the final docker-compose.yml file that will be used after setup
        compose_file = os.path.join(install_path, 'docker-compose.yml')
        with open(compose_file, 'w') as f:
            f.write(f"""version: '3'
services:
...
""")
        ...
        with open(setup_script, 'w') as f:
            f.write(f"""#!/bin/bash
echo "Stopping temporary Filestash container..."
docker-compose -f docker-compose-before.yml down

echo "Copying state from temporary container..."
mkdir -p ./state
cp -R ./data/* ./state/

echo "Starting final Filestash container with OnlyOffice..."
docker-compose up -d

echo "Setup complete! Filestash is now available at http://$(hostname -I | awk '{{print $1}}'):{port}"
""")
        
        # Make the script executable
        os.chmod(setup_script, 0o755)
        
        # Initial setup uses the temporary docker-compose-before.yml
        cmd = f"cd {install_path} && docker-compose -f docker-compose-before.yml up -d"
        subprocess.run(cmd, shell=True, check=True)
        
        host_ip = get_local_ip()
        logger.info(f"Started temporary Filestash container. Please visit http://{host_ip}:{port} to complete the setup")
        logger.info(f"After setup, run {setup_script} to finalize the installation")
